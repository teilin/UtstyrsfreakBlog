...
class Vcard_Creator {
    private static function getVcf($data, $version = 2.1) {
        $ret = "BEGIN:VCARD\r\n";
        $ret .= "VERSION:" . $version . "\r\n";
        $ret .= isset($data['name']) ? "N:" . $data['name'] . "\r\n" : "";
        $ret .= isset($data['name']) ? "FN:" . $data['name'] . "\r\n" : "";
        $ret .= isset($data['company']) ? "ORG:" . $data['company'] . "\r\n" : "";
        $ret .= isset($data['address']) ? "ADR;WORK:" . $data['address'] . "\r\n" : "";
        $ret .= isset($data['phone']) ? "TEL;WORK;VOICE;PREF:" . $data['phone'] . "\r\n" : "";
        $ret .= isset($data['fax']) ? "TEL;WORK;FAX:" . $data['fax'] . "\r\n" : "";
        $ret .= isset($data['email']) ? "EMAIL;INTERNET;PREF:" . $data['email'] . "\r\n" : "";
        $ret .= isset($data['url']) ? "URL;WORK;PREF:" . $data['url'] . "\r\n" : "";
        $ret .= isset($data['note']) ? "NOTE:" . $data['note'] . "\r\n" : "";
        if ($version == 3)
            $ret .= isset($data['icon']) ? "PHOTO;VALUE=URL;TYPE=PNG:" . $data['icon'] . "\r\n" : "";
        $ret .= "REV:3\r\n";
        $ret .= "END:VCARD\r\n";
        return $ret;
    }
